...
If (FileExists (@ScriptDir&"\app32\virtualbox.exe") OR FileExists (@ScriptDir&"\app64\virtualbox.exe")) AND ($startvbox = 1 OR IniRead (@ScriptDir&"\data\settings\vboxinstall.ini", "startvbox", "key", "NotFound") = 1) Then
  ...
  If FileExists (@ScriptDir&"\"&$UserHome&"\VirtualBox.xml") OR (FileExists (@ScriptDir&"\"&$UserHome&"\Machines\") AND FileExists (@ScriptDir&"\"&$UserHome&"\HardDisks\")) Then
    ...
    EndIf
  Else
    If IniRead ($var1, "userhome", "key", "NotFound") = 1 Then
...
Func UseSettings ()
  ...
  If GUICtrlRead ($Checkbox100) = $GUI_CHECKED AND FileExists (@ScriptDir&"\temp") Then
    ...
    FileDelete (@ScriptDir&"\app32\ExtensionPacks\Oracle_VM_VirtualBox_Extension_Pack\win.amd64\*.r0")
    FileDelete (@ScriptDir&"\app32\*.r0")
    FileDelete (@ScriptDir&"\app32\*.qch")
    FileDelete (@ScriptDir&"\app32\*.qhc")
  ...
  If GUICtrlRead ($Checkbox110) = $GUI_CHECKED AND FileExists (@ScriptDir&"\temp") Then
    ...
    FileDelete (@ScriptDir&"\app64\ExtensionPacks\Oracle_VM_VirtualBox_Extension_Pack\win.amd64\*.r0")
    FileDelete (@ScriptDir&"\app64\*.r0")
    FileDelete (@ScriptDir&"\app64\*.qch")
    FileDelete (@ScriptDir&"\app64\*.qhc")
