...
If FileExists (@TempDir&"\update.ini") Then
  ...
  Endif
Else
  IniReadSection (@ScriptDir&"\data\settings\settings.ini", "version")
  If @error Then
    IniWrite ($var1, "version", "key", "")
  EndIf
  
  IniReadSection (@ScriptDir&"\data\settings\settings.ini", "starter")
  If @error Then
    IniWrite ($var1, "starter", "key", "")
  EndIf
  
  IniReadSection (@ScriptDir&"\data\settings\settings.ini", "update")
  If @error Then
...
If $new1 = 1 OR $new2 = 1 Then
  ...
  If @OSLang = "0407" OR @OSLang = "0807" OR @OSLang = "0c07" OR @OSLang = "1007" OR @OSLang = "1407" Then
    If $new1 = 1 AND $new2 = 0 Then
      Global $dialog = "Eine neue VirtualBox-Version "& $ov_d &" auf "& $nv_d &" ist verfügbar. Willst du jetzt aktualisieren?"
    EndIf

    If $new1 = 0 AND $new2 = 1 Then
      Global $dialog = "Eine neuer Starter "& $os_d &" auf "& $ns_d &" ist verfügbar. Willst du ihn jetzt aktualisieren?"
    EndIf

    If $new1 = 1 AND $new2 = 1 Then
      Global $dialog = "Eine neue Version von VirtualBox "& $ov_d &" auf "& $nv_d &" und dem Starter "& $os_d &" auf "& $ns_d &" sind verfügbar. Willst du jetzt aktualisieren?"
    EndIf
    
    Local $dialog_u1 = "Portable-VirtualBox *** Aktualisieren JA oder NEIN ***"
    Local $dialog_u2 = "alte Dateien sichern"
    Local $dialog_u3 = "JA"
    Local $dialog_u4 = "NEIN"
  Else
    If $new1 = 1 AND $new2 = 0 Then
      Global $dialog = "A new VirtualBox-Version "& $ov_d &" to "& $nv_d &" is available. Do you want to update now?"
    EndIf

    If $new1 = 0 AND $new2 = 1 Then
      Global $dialog = "New Launcher "& $os_d &" to "& $ns_d &" is available. Do you want to update it now?"
    EndIf

    If $new1 = 1 AND $new2 = 1 Then
      Global $dialog = "A new version of VirtualBox "& $ov_d &" to "& $nv_d &" and the Launcher "& $os_d &" to "& $ns_d &" are available. Do you want to update? now"
    EndIf
    
    Local $dialog_u1 = "Portable-VirtualBox *** UpDate YES or NO ***"
    Local $dialog_u2 = "backup old files"
    Local $dialog_u3 = "YES"
    Local $dialog_u4 = "NO"
  EndIf
  
  GUICreate ($dialog_u1, 300, 190, -1, -1, $WS_POPUP)
  ...
  $Checkbox200 = GUICtrlCreateCheckbox ($dialog_u2, 14, 62, 260, 14)
  
  GUICtrlCreateLabel ("Status-Info:", 14, 82, 280, 10)
  GUICtrlSetFont (-1, 8, 800, 4,"Arial")
  $Input300 = GUICtrlCreateLabel ("", 14, 96, 260, 20)
  GUICtrlSetFont (-1, 8, 400, 0,"Arial")
  
  GUICtrlCreateButton ($dialog_u3, 32, 116, 100, 33, 0)
  GUICtrlSetFont (-1, 9, 800, "Arial")
  GUICtrlSetOnEvent (-1, "UpdateYes")
  GUICtrlCreateButton ($dialog_u4, 160, 116, 100, 33, 0)
  GUICtrlSetFont (-1, 9, 800, "Arial")
...
If NOT (FileExists (@ScriptDir&"\app32") OR FileExists (@ScriptDir&"\app64")) Then
  ...
  If @OSLang = "0407" OR @OSLang = "0807" OR @OSLang = "0c07" OR @OSLang = "1007" OR @OSLang = "1407" Then
    Local $dialog_up0 = "Portable-VirtualBox *** Entpacken und/oder Komprimieren ***"
    Local $dialog_up1 = "Hier kannst du die Installationsdateien von VirtualBox automatisch herunterladen lassen und einige Einstellungen vornehmen (Wenn du VirtualBox mit diesem Skript herunterlädst, nur noch die Einstellungen vornehmen und auf ""OK"" klicken)."
    Local $dialog_up2 = "Installationsdateien von VirtualBox herunterladen"
    Local $dialog_up3 = "ODER"
    Local $dialog_up4 = "Pfad zu der Installationsdatei von VirtualBox ist ..."
    Local $dialog_up5 = "Entpacke die Dateien für ein 32-Bit-System"
    Local $dialog_up6 = "Entpacke die Dateien für ein 64-Bit-System"
    Local $dialog_up7 = "Komprimiere die Dateien, um Platz zu sparen"
    Local $dialog_up8 = "Starte Portable-VirtualBox nach dem entpacken und/oder komprimieren"
    Local $dialog_up9 = "Lizenz von VirtualBox"
  Else
    Local $dialog_up0 = "Portable-VirtualBox *** Extract and/or Compress ***"
    Local $dialog_up1 = "Here you can find the installation files of VirtualBox can automatically download and make some adjustments (If you use this script downloading VirtualBox, only make the settings and click ""OK"")."
    Local $dialog_up2 = "Download installation files of VirtualBox"
    Local $dialog_up3 = "OR"
    Local $dialog_up4 = "Path to the installation file of VirtualBox is ..."
    Local $dialog_up5 = "Extract the files for a 32-Bit system"
    Local $dialog_up6 = "Extract the files for a 64-Bit system"
    Local $dialog_up7 = "Compress the files to reduce the size"
    Local $dialog_up8 = "Start Portable-VirtualBox after the extract and/or compress"
    Local $dialog_up9 = "License of VirtualBox"
  EndIf
  
  GUICreate ($dialog_up0, 542, 380, -1, -1, $WS_POPUP)
  GUISetFont (9, 400, 0, "Arial")
  GUISetBkColor (0xFFFFFF)
  GUICtrlSetFont (-1, 10, 800, 0, "Arial")
  
  GUICtrlCreateLabel ($dialog_up1, 32, 8, 476, 60)
  GUICtrlSetFont (-1, 9, 800, "Arial")
  
  GUICtrlCreateButton ($dialog_up2, 32, 62, 473, 33)
  GUICtrlSetFont (-1, 14, 400, "Arial")
  GUICtrlSetOnEvent (-1, "DownloadFile")
  
  GUICtrlCreateLabel ($dialog_up3, 250, 101, 80, 40)
  GUICtrlSetFont (-1, 10, 800, "Arial")
  
  $Input100 = GUICtrlCreateInput ($dialog_up4, 32, 124, 373, 21)
  GUICtrlCreateButton ("Suche Datei", 412, 122, 93, 25, 0)
  GUICtrlSetOnEvent (-1, "SearchFile")
  
  $Checkbox100 = GUICtrlCreateCheckbox ($dialog_up5, 32, 151, 460, 26)
  $Checkbox110 = GUICtrlCreateCheckbox ($dialog_up6, 32, 175, 460, 26)
  $Checkbox120 = GUICtrlCreateCheckbox ($dialog_up7, 32, 199, 460, 26)
  $Checkbox130 = GUICtrlCreateCheckbox ($dialog_up8, 32, 223, 460, 26)
  
  GUICtrlCreateLabel ("Status-Info:", 32, 247, 436, 26)
  GUICtrlSetFont (-1, 8, 800, 4,"Arial")
  $Input200 = GUICtrlCreateLabel ("", 32, 264, 476, 47)
  GUICtrlSetFont (-1, 8, 400, 0,"Arial")
  
  GUICtrlCreateButton ("OK", 52, 308, 129, 33, 0)
  GUICtrlSetOnEvent (-1, "UseSettings")
  GUICtrlCreateButton ($dialog_up9, 194, 308, 149, 33, 0)
  GUICtrlSetFont (-1, 8, 600, "Arial")
  GUICtrlSetOnEvent (-1, "Licence")
  GUICtrlCreateButton ("Exit", 356, 308, 129, 33, 0)
...
If (FileExists (@ScriptDir&"\app32\virtualbox.exe") OR FileExists (@ScriptDir&"\app64\virtualbox.exe")) AND ($startvbox = 1 OR IniRead (@ScriptDir&"\data\settings\vboxinstall.ini", "startvbox", "key", "NotFound") = 1) Then
  ...
  Else
    If @OSLang = "0407" OR @OSLang = "0807" OR @OSLang = "0c07" OR @OSLang = "1007" OR @OSLang = "1407" Then
      MsgBox (0, "Info", "Bitte starte VirtualBox neu, damit alle Pfade angepasst werden können!")
    Else
      MsgBox (0, "Info", "Please start VirtualBox again, so that all paths can be adapted!")
    EndIf
  EndIf
  ...
  If FileExists (@ScriptDir&"\"& $arch & "\VirtualBox.exe") AND FileExists (@ScriptDir&"\"& $arch & "\VBoxSVC.exe") AND FileExists (@ScriptDir&"\"& $arch & "\VBoxC.dll") Then
    If NOT ProcessExists ("VirtualBox.exe") OR NOT ProcessExists ("VBoxManage.exe") Then
      ...
      If IniRead ($var1, "hotkeys", "key", "NotFound") = 1 Then
        ...
        TraySetState ()
	TraySetToolTip ("Right-click to see the menu!")
	TrayTip("", "Right-click to see the menu!", 5)
      Else
...
Func DownloadFile ()
  ...
  Do
    ...
    GUICtrlSetData ($Input200, "Downloading: " & @LF & $download2 & @LF & "Bytes = " & $bytes)
  ...
  EndIf
  GUICtrlSetData ($Input200, @LF & "Ready with the download! The files was stored in the Portable-VirtualBox folder.")
EndFunc

Func SearchFile ()
  Local $FilePath = FileOpenDialog ("VirtualBox - Installation File", @ScriptDir, "(*.exe)", 1+2)
  If NOT @error Then
...
Func UseSettings ()
  If GUICtrlRead ($Input100) = "" OR GUICtrlRead ($Input100) = "Pfad zu der Installationsdatei von VirtualBox ist ..." OR GUICtrlRead ($Input100) = "Path to the installation file of VirtualBox is ..." Then
  ...
  If (FileExists (@ScriptDir&"\virtualbox.exe") OR FileExists ($SourceFile)) AND (GUICtrlRead ($Checkbox100) = $GUI_CHECKED OR GUICtrlRead ($Checkbox110) = $GUI_CHECKED) Then
    GUICtrlSetData ($Input200, @LF & "Please wait, extract files.")
    If FileExists (@ScriptDir&"\virtualbox.exe") Then
  ...
  If GUICtrlRead ($Checkbox100) = $GUI_CHECKED AND FileExists (@ScriptDir&"\temp") Then
    GUICtrlSetData ($Input200, @LF & "Please wait, copy files.")
  ...
  If GUICtrlRead ($Checkbox120) = $GUI_CHECKED Then
    GUICtrlSetData ($Input200, @LF & "Please wait, compress files.")
  ...
  If (FileExists (@ScriptDir&"\virtualbox.exe") OR FileExists ($SourceFile)) AND (GUICtrlRead ($Checkbox100) = $GUI_CHECKED OR GUICtrlRead ($Checkbox110) = $GUI_CHECKED) Then
    GUICtrlSetData ($Input200, @LF & "Finished, extracts and/or compress and copy files.")
    Sleep (2000)
  EndIf
...
Func UpdateYes ()
  If $new1 = 1 Then
    ...
    If GUICtrlRead ($Checkbox200) = $GUI_CHECKED Then
      GUICtrlSetData ($Input300, "Please wait, make backup of old files and folders.")
    ...
    Else
      GUICtrlSetData ($Input300, "Please wait, delete old files and folders.")
  ...
  If $new2 = 1 Then
    ...
    GUICtrlSetData ($Input300, "Please wait, download UpDate.")
    ...
    GUICtrlSetData ($Input300, "Please wait, Extracts files.")
    Sleep (2000)
    ...
    GUICtrlSetData ($Input300, "Please wait, copy files.")
    Sleep (2000)
  ...
  GUICtrlSetData ($Input300, "Finished, delete or backup of files.")
  Sleep (2000)
